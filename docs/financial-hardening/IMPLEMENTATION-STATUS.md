...
- Independent scalar oracle in tests uses own arithmetic/tax formula only (no production
  survival/leaf financial helpers), own bounding/binary loop; terminal trivial for bank-only
  (deposit-only liquidation gain is zero, so production terminal always passes when yearly passes);
  agrees within `€1`.
...
- `investmentTax/validation.ts`: `checked()` is a structured fast path with the same numeric coverage as `checkedFull()` for the known `InvestmentState` shape (every number checked field-by-field via `finite()` plus `totalValue()` and bucket-level `nonnegative()`); `checkedFull()` keeps the recursive `numbers()` walk for tests only; parity is covered by `structuralSharing.test.ts`. No `WeakMap`/watermarks. Known narrow gap: extra unknown numeric props that `checkedFull()` would catch via the recursive sweep are missed by `checked()`; no financial impact today (unknown props are ignored downstream). `finishTransactionBatch()` always delegates to `checked()` (no suffix-only path; batch counts ignored). `cloneInvestmentState()` is handwritten deep copy element-wise (`buckets/cohorts`, `taxYears`, `pending/amounts`, `taxIncome`, `contributionIncome`, `transactions`, `eventIds`); no shared mutable audit records, no `structuredClone` in production. `transition()` still validates input then clones.
...
- `bankOnlySearchHardening.test.ts`: 9 tests (was 6 at the time of `green-bank-search.log`; 3 added: low-artefact, true-zero, signed-rates-with-church) — initial red 4 failed (`isBankOnlySearchSupported is not a function`,
  `openingForCapital is not a function`), green 9 passed after fix. Saved `green-bank-search.log` records the earlier 6-test run (historical).
- Parity suites (no saved gate receipts in this folder at `05592c6`): `ledgerBatchingParity` 13, `insuranceHoistingParity` 10, `lifecycleDedupParity` 2, `registryHoistingParity` 3, `validationMemoParity` 3.
- `reviewFixFinal.test.ts`: 7 tests locking REVIEW-FINAL B1/B2/B3 plus N4 key encoding (all 7 fail on `05592c6`, all 7 pass after fix).
...
- `GOMAXPROCS=2 npx vitest run --maxWorkers=1` (full), `npm run lint`, `npx tsc -b --force`, `npm run build`,
  `git diff --check`. No committed `gates.log` in this folder (only `green-*.log`/`red-*.log`); exact current
  counts are recorded in `REVIEW-FIX-FINAL-STATUS.md` outside the repo. The `05592c6` message claim
  `FULL 733/733 in 129.5s` is historical and is not re-claimed here without a committed receipt.
