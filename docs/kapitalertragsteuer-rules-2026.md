# Kapitalertragsteuer auf Rentenphasen-Entnahmen — Rule snapshot (PR tax-1)

Scope: taxation of portfolio withdrawals that fund the retirement gap
(`gapWithdrawal` rows in the yearly ledger). Implements Abgeltungsteuer as a
...
**Reads (per year):**
- `gapWithdrawal` — the withdrawal amount to fund (Ruhestand rows only).
...
resulting capital income → tax funding from the portfolio → remaining withdrawal.
...
| Sparerpauschbetrag | 1,000 EUR/year (single), applies after Teilfreistellung, before the 25% rate; consumed first, never refunded across years | [§20(9) EStG](https://www.gesetze-im-internet.de/estg/__20.html) |
...
- Tax on capital income *outside* the gap-withdrawal flow (dividends are not
  supported holdings anyway — accumulating funds and bank deposits only).
