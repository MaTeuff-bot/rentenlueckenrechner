...
export function CapitalEstimatorSetup({ insurance, onChange }: {
  ...
  return <fieldset><legend>Automatische Kapitalertragsschätzung</legend>
    ...
    <OptionalNumber label="Anschaffungskosten des gesamten Fondspools (€)" value={setup.fundAcquisitionCost} onChange={fundAcquisitionCost => update({ fundAcquisitionCost })} />
    ...
    <details><summary>Erweitert – projizierter Basiszins</summary>
      <OptionalNumber label="Konstanter nominaler Basiszins (%)" value={setup.projectedBasisRate * 100} min={-100} max={100} onChange={v => update({ projectedBasisRate: v === undefined ? NaN : v / 100 })} />
